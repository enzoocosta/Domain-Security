...
from app.schemas.discovery import DiscoveryRunCreateInput, DiscoveryRunDetail, DiscoveryRunSummary
...
@router.get("/discovery", response_model=list[DiscoveryRunSummary], summary="Listar execucoes de asset discovery")
...
@router.post("/discovery", response_model=DiscoveryRunDetail, status_code=status.HTTP_201_CREATED, summary="Executar asset discovery")
def create_discovery_run(payload: DiscoveryRunCreateInput, request: Request) -> DiscoveryRunDetail:
    current_user = auth_service.get_user_session(request)
    if current_user is None:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Autenticacao requerida.")
    try:
...
@router.get("/discovery/{run_id}", response_model=DiscoveryRunDetail, summary="Detalhar execucao de asset discovery")
