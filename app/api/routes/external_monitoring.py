...
def create_monitoring(
    ...
    try:
        ...
        raise HTTPException(status_code=get_http_status_code(exc), detail=str(exc)) from exc
    return ExternalMonitoringMutationResponse(message="Monitoramento criado.", item=item)
...
@router.get("/monitoring/{monitored_domain_id}", response_model=ExternalMonitoringDetailResponse)
...
@router.post("/monitoring/{monitored_domain_id}/pause", response_model=ExternalMonitoringMutationResponse)
def pause_monitoring(
    ...
    try:
        ...
        raise HTTPException(status_code=get_http_status_code(exc), detail=str(exc)) from exc
    return ExternalMonitoringMutationResponse(message="Monitoramento pausado.", item=item)


@router.post("/monitoring/{monitored_domain_id}/resume", response_model=ExternalMonitoringMutationResponse)
def resume_monitoring(
    ...
    try:
        ...
        raise HTTPException(status_code=get_http_status_code(exc), detail=str(exc)) from exc
    return ExternalMonitoringMutationResponse(message="Monitoramento retomado.", item=item)


@router.delete("/monitoring/{monitored_domain_id}", response_model=ExternalMonitoringMutationResponse)
def delete_monitoring(
    ...
    try:
        ...
        raise HTTPException(status_code=get_http_status_code(exc), detail=str(exc)) from exc
    return ExternalMonitoringMutationResponse(message="Monitoramento excluido.", item=item)
