...
_RSS_VERSION_RE = re.compile(
    r"wordpress(?:\.org)?/?\?v=([0-9A-Za-z.\-_]+)", re.IGNORECASE
)
...
class WordPressSecurityService:
    ...
    def __init__(
        self, *, cache_ttl_seconds: int = 3600, timeout_seconds: float = 5.0
    ) -> None:
    ...
    def analyze_site(
        ...
        if cached is not None:
            return WordPressAnalysisResponse.model_validate(
        ...
        if site_confirmed and (selected.detect_plugins or selected.detect_themes):
            ...
            if selected.detect_plugins:
                ...
                plugin_versions = self._extract_asset_versions(
                    _PLUGIN_VERSION_RE, home_html
                )
            if selected.detect_themes:
                theme_slugs = self._extract_unique_slugs(_THEME_RE, home_html)
                theme_versions = self._extract_asset_versions(
                    _THEME_VERSION_RE, home_html
                )
    ...
    def _detect_wordpress(
        self, scanned_url: str, home_page: FetchedPage
    ) -> WordPressDetectionResult:
        checks: list[tuple[int, str, Callable[[], DetectionOutcome]]] = [
            ...
            (
                ...
                lambda: self._check_asset_paths(home_page.text),
            ),
            ...
            (
                10,
                "Body classes do WordPress",
                lambda: self._check_body_classes(home_page.text),
            ),
        ]
        ...
        with ThreadPoolExecutor(max_workers=len(checks)) as executor:
            futures = {
                executor.submit(check): (layer, name) for layer, name, check in checks
            }
    ...
    def _check_wp_login(self, url: str) -> DetectionOutcome:
        page = self._fetch_optional_page(
            urljoin(self._base_url(url), "wp-login.php"), method="HEAD"
        )
        detected = page is not None and page.status_code in {
            200,
            301,
            302,
            303,
    ...
    def _check_xmlrpc(self, url: str) -> DetectionOutcome:
        page = self._fetch_optional_page(
            urljoin(self._base_url(url), "xmlrpc.php"), method="HEAD"
        )
    ...
    def _check_wp_admin(self, url: str) -> DetectionOutcome:
        page = self._fetch_optional_page(
            urljoin(self._base_url(url), "wp-admin/"), method="HEAD"
        )
    ...
    def _check_body_classes(self, html: str) -> DetectionOutcome:
        ...
        if not match:
            return DetectionOutcome(
                layer=10, name="Body classes do WordPress", detected=False
            )
    ...
    def _version_source_from_signals(
        self, signals: list[WordPressDetectionSignal]
    ) -> str | None:
    ...
    def _build_component_item(
        ...
        *,
        ...
        if (
            not vulnerabilities
            and version_detected is None
            and (data.get("vulnerability") or [])
        ):
            status = "atencao"
        ...
        *,
        ...
        for entry in entries:
            ...
            if (
                version_detected
                and operator
                and not self._version_matches_operator(version_detected, operator)
            ):
                continue

            severity, score = self._extract_severity_and_score(
                entry.get("impact") or {}
            )
            ...
            title = unescape(
                source.get("name") or entry.get("name") or "Vulnerabilidade conhecida"
            )
            vulnerability_id = str(
                entry.get("uuid") or source.get("id") or title
            ).strip()
    ...
    def _extract_severity_and_score(
        ...
        severity = (
            str(cvss3.get("severity") or cvss.get("severity") or "").strip().lower()
        )
        if severity in {"critical", "high", "medium", "low"}:
            return severity, score
        if severity in {"c", "h", "m", "l"}:
            return {"c": "critical", "h": "high", "m": "medium", "l": "low"}[
                severity
            ], score
        if score is None:
            return "medium", None
    ...
    def _status_for_vulnerabilities(
        self, vulnerabilities: list[WordPressVulnerability]
    ) -> str:
    ...
    def _normalize_url(self, value: str) -> str:
        ...
        if not raw:
            raise InputValidationError(
                "Informe a URL do site WordPress que voce deseja analisar."
            )

        parsed = urlparse(raw if "://" in raw else f"https://{raw}")
        if parsed.scheme not in {"http", "https"} or not parsed.netloc:
            raise InputValidationError(
                "Informe uma URL valida, por exemplo https://meusite.com.br."
            )
        ...
        for element in root.iter():
            ...
            if tag.endswith("generator") and element.text:
                direct_match = re.search(
                    r"wordpress\s+([0-9A-Za-z.\-_]+)", element.text, re.IGNORECASE
                )
                if direct_match:
    ...
    def _extract_asset_versions(
        self, pattern: re.Pattern[str], html: str
    ) -> dict[str, str]:
    ...
    def _compare_versions(self, left: str, right: str) -> int:
        ...
        for index in range(max_len):
            ...
            return (
                -1
                if self._version_part_key(left_part)
                < self._version_part_key(right_part)
                else 1
            )
        return 0
