...
class AnalysisSnapshot(Base):
    ...
    analysis_domain: Mapped[str] = mapped_column(
...
class MonitoredDomain(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "user_id", "normalized_domain", name="uq_monitored_domains_user_domain"
        ),
    )
    ...
    plan: Mapped[str] = mapped_column(
        String(16), nullable=False, default="standard", index=True
    )
    ...
    last_alert_reason: Mapped[str | None] = mapped_column(String(255), nullable=True)
    alert_contacts: Mapped[list[str]] = mapped_column(
    ...
    def last_checked_at(self, value: datetime | None) -> None:
        self.last_run_at = value

    @property
    def check_results(self) -> list["MonitoringRun"]:
        return self.monitoring_runs
...
class MonitoringRun(Base):
    ...
    alerts_fired: Mapped[list[str]] = mapped_column(JSON, nullable=False, default=list)
    ...
    analysis_snapshot: Mapped[AnalysisSnapshot | None] = relationship(
        back_populates="monitoring_runs"
    )
    alert_events: Mapped[list["AlertEvent"]] = relationship(
...
class DiscoveredSubdomain(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "discovery_run_id", "fqdn", name="uq_discovered_subdomains_run_fqdn"
        ),
    )
...
class PremiumSubscription(Base):
    ...
    plan_code: Mapped[str] = mapped_column(
        String(32), nullable=False, default="monitoring_plus"
    )
    status: Mapped[str] = mapped_column(
    ...
    monitored_domain: Mapped[MonitoredDomain] = relationship(
        back_populates="premium_subscription"
    )
...
class PremiumIngestToken(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "token_identifier", name="uq_premium_ingest_tokens_identifier"
        ),
    )
    ...
    monitored_domain: Mapped[MonitoredDomain] = relationship(
        back_populates="premium_ingest_tokens"
    )
...
class TrafficIncident(Base):
    ...
    monitored_domain: Mapped[MonitoredDomain] = relationship(
        back_populates="traffic_incidents"
    )


# Compatibility aliases used by the new APScheduler integration and deploy docs.
MonitoringTarget = MonitoredDomain
MonitoringCheckResult = MonitoringRun
